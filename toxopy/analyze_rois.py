...
def analyze_rois(file, room_layout, output_dir, trial_type, span=10):
    ...
    if output_dir.endswith("/") is False:
        ...
        raise ValueError('Output directory does not end with a trailing slash "/"!')
    pass
    ...
    for trial, trial_name in zip(trials, trials_names):
        ...
        if trial_type == "with_owner":
            ...
            return {
                "middle": position(
                    (p[i]["middle_tl"][0], p[i]["middle_tl"][1]),
                    (p[i]["middle_br"][0], p[i]["middle_br"][1]),
                ),
                "rightside": position(
                    (p[i]["right_tl"][0], p[i]["right_tl"][1]),
                    (p[i]["right_br"][0], p[i]["right_br"][1]),
                ),
                "leftside": position(
                    (p[i]["left_tl"][0], p[i]["left_tl"][1]),
                    (p[i]["left_br"][0], p[i]["left_br"][1]),
                ),
                "topside": position(
                    (p[i]["top_tl"][0], p[i]["top_tl"][1]),
                    (p[i]["top_br"][0], p[i]["top_br"][1]),
                ),
                "bottomside": position(
                    (p[i]["bottom_tl"][0], p[i]["bottom_tl"][1]),
        ...
        res = time_in_each_roi.get_timeinrois_stats(
            bp_tracking.T, rois, fps=30, returndf=True, check_inroi=True
        )
        ...
        df_final.to_csv(
            output_dir + cat + trial_name + "_deleteme" + ".csv",
            index=False,
            sep=",",
            encoding="utf-8",
        )
    ...
    combined_csv = combined_csv.iloc[combined_csv["trial"].map(sort_by_trial).argsort()]

    if trial_type == "with_owner":

        combined_csv.to_csv(
            output_dir + cat + "_with_owner_rois.csv", index=False, encoding="utf-8-sig"
        )

    elif trial_type == "cat_alone":

        combined_csv.to_csv(
            output_dir + cat + "_cat_alone_rois.csv", index=False, encoding="utf-8-sig"
        )
